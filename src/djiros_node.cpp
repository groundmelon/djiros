...
/* ros sub from serial */
ros::Subscriber cmd_data_sub,nav_open_close_sub, ctrl_mode_sub, ctrl_data_sub, simple_task_sub, activation_sub, ctrl_sub;
...
int16_t nav_force_close_handler(uint8_t cmd_id,uint8_t* pbuf,uint16_t len,req_id_t req_id)
{
	...
	ROS_ERROR("WARNING nav close by app %d !!!!!!!!!!!!!! \n", msg);
	return 0;
...
{
	uint8_t buf = 0;
	App_Send_Data(1, 1, MY_CTRL_CMD_SET, API_OPEN_SERIAL, (uint8_t*)&buf, sizeof(uint8_t), sdk_ack_nav_open_close_callback,  1000, 0);
	ROS_INFO("Try to release control.");
	usleep(500*1000);
	ROS_ERROR("Exit...");
	ros::shutdown();
}
...
int main (int argc, char** argv)
{
	...
	{
		ROS_ERROR("[ERR] activation failed.");
		return 9;
	}
...
void api_acquire_control()
{
	uint8_t buf = 1;
	App_Send_Data(1, 1, MY_CTRL_CMD_SET, API_OPEN_SERIAL, (uint8_t*)&buf, sizeof(uint8_t), sdk_ack_nav_open_close_callback,  1000, 0);
	ROS_INFO("Try to acquire control.");
}
