#ifndef __HARDDRIVERROS_H
#define __HARDDRIVERROS_H 1
...
class HardDriverRos : public HardDriver {
  public:
    HardDriverRos(std::string device, unsigned int baudrate) {
        m_device = device;
        m_baudrate = baudrate;
    ...
    void init() {
        if (m_serial_fd < 0) {
            if (_serialStart(m_device.c_str(), m_baudrate) < 0) {
                // ROS_ERROR("[djiros] Fail to start %s", m_device.c_str());
                _serialClose();
            } else {
                ROS_INFO("[djiros] Serial %s is opened\n", m_device.c_str());
            }
        }
    }
...
// #endif
  ...
  private:
    ...
    bool _serialOpen(const char *dev) {
        ...
        if (m_serial_fd < 0) {
            API_LOG(this, ERROR_LOG, "cannot open device %s : errno[%d] %s\n", dev, errno, strerror(errno));
            return false;
        }
        return true;
    }
    ...
    bool _serialConfig(int baudrate, char data_bits, char parity_bits, char stop_bits) {
        ...
        int std_rate[] = {
            4800, 9600, 19200, 38400, 57600, 115200, 230400, 921600, 1000000, 1152000, 3000000,
        };
        ...
        if (tcgetattr(m_serial_fd, &oldtio) != 0) {
            API_LOG(this, ERROR_LOG, "fail to save current port : fd:%d errno[%d] %s\n", m_serial_fd, errno, strerror(errno));
            // return false;
        }
